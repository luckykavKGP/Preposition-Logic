...
// popping out an element from stack
...
// defining BinaryTree Data Structure
...
// Initiating a node in binary tree
BT *InitNode (char el)
{
    BT *newNode;
    if ( (newNode=(BT *)malloc(sizeof(BT))) == NULL )
        ...
        newNode->element = el;      // Value present in the Node
        newNode->leftChild = NULL;  // leftChild of node
        newNode->rightChild = NULL; // rightChild of node
    }
    return (newNode);
}

// Creating Duplicate Copy of the Binary Tree
BT *duplicate(BT *orig)
{
    BT *dupli = NULL;
    if(orig != NULL) {
        dupli = InitNode(orig->element);
        dupli->leftChild = duplicate(orig->leftChild);
        dupli->rightChild = duplicate(orig->rightChild);
    }
    return (dupli);
}

// structure for holding propositional values
typedef struct ProVal {
    char prop;
...
{
    BT *et = NULL;
    if(end-start+1 == 1) et = InitNode(pfForm[end]);
